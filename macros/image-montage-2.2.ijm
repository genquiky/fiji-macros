// Image Montage v.2
// January 26, 2014 (Updated March 2024)
	...
	if(use_gui == 1){
		...
		if(cat_list_flag == 0){
			...
			for (i=1; i<list.length; i++) {
				...
				if (endsWith(list[i-1], ".jpg") || endsWith(list[i-1],".png") || endsWith(list[i-1],".JPG") || endsWith(list[i-1],".PNG") || endsWith(list[i-1],".tif") || endsWith(list[i-1],".TIF")) {
					count++;
				}
			}
...
function countFiles(dir) {
	...
	for (i=0; i<list.length; i++) {
		...
		} else if(endsWith(list[i], ".jpg") || endsWith(list[i],".png") || endsWith(list[i],".JPG") || endsWith(list[i],".PNG") || endsWith(list[i],".tif") || endsWith(list[i],".TIF")) {
			count++;
		}
	}
}
...
if( input_flag == 0 || input_flag == 2 || input_flag == 3){
	for(j=0; j<list.length; j++){
		if(endsWith(list[j],".jpg") || endsWith(list[j],".png") || endsWith(list[j],".JPG") || endsWith(list[j],".PNG") || endsWith(list[j],".tif") || endsWith(list[j],".TIF")){
			if(input_flag == 3){
